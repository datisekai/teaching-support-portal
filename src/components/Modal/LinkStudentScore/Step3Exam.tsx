...
const Step3Exam: React.FC<Props> = ({ hashCurrentScore, hashScore, setHashScore }) => {
    ...
    const tableSchemas = useMemo(() => {
        return [
            ...
            },
            {
                ...
                width: 100,
                render: (data: any) => {
                    return <div>{hashCurrentScore[data.code]?.[content.id]?.score?.toFixed(2) || 0}</div>
                }
            },
            {
                label: "Điểm mới",
                ...
                width: 100,
                render: (data: any) => {
                    return <InputNumber minFractionDigits={2} min={0} max={10} className='input-modal-link' onValueChange={e => setHashScore({ ...hashScore, [data.code]: { ...hashScore[data.code], grade: e.value } })} value={hashScore[data.code]?.grade || 0} />
                }
            },
            {
                label: "Hành động",

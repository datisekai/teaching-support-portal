...
const AttendanceModal = () => {
    ...
    useEffect(() => {
        ...
        socket?.emit(SocketMessage.JOIN_OR_CREATE, { expirationTime: content.expirationTime || 3000, classId: content.class.id, secretKey: content.secretKey, id: content.id, attendees: content.attendees.map((item: any) => ({ code: item.user.code, name: item.user.name, time: new Date(item.createdAt).getTime() })) }, (response: ISocketResponse) => {
    ...
    const handleUpdateStatus = (isOpen: boolean) => {
        socket?.emit(SocketMessage.UPDATE_STATUS_ROOM, { ...roomState, isOpen }, (response: ISocketResponse) => {
    ...
    return (
        <div className='tw-flex  tw-gap-2'>
                ...
                <TabView>
                    <TabPanel header="Thông tin">
                        <div className='tw-space-y-1'>
                            ...
                            <div>
                                Trạng thái: <Tag severity={roomState.isOpen ? 'info' : 'warning'} value={roomState.isOpen ? 'Đang điểm danh' : "Không hoạt động"}></Tag>
                            </div>
                        </div>
                        <div className='tw-flex  tw-flex-wrap tw-items-center tw-gap-2 tw-mt-4'>
                            <Button onClick={() => handleUpdateStatus(true)} disabled={roomState.isOpen} severity='info' label='Bắt đầu'></Button>
                            <Button onClick={() => handleUpdateStatus(false)} disabled={!roomState.isOpen} severity='danger' label='Huỷ'></Button>
                        </div>
                    </TabPanel>
                    <TabPanel header="Danh sách">
                        <MyTable schemas={AttendeeSchemas} data={roomState.attendees || []} />
                    </TabPanel>
            ...
            <div className='tw-aspect-[1/1] relative p-4  tw-flex-1 tw-border tw-rounded-lg tw-flex tw-justify-center tw-items-center'>
                ...
                {roomState.isOpen && roomState.qrCode && <div className='tw-w-full'>
                    <QRCode style={{ height: "auto", maxWidth: "100%", width: "100%" }} value={roomState.qrCode} /></div>}
                {!roomState.isOpen && !roomState.qrCode && <div className='tw-font-semibold'>Để điểm danh, hãy click bắt đầu</div>}
                {!roomState.isOpen && roomState.qrCode && <div className='tw-font-semibold'>Thời gian điểm danh đã kết thúc</div>}
            </div>

            <div className='tw-absolute tw-bottom-2 tw-left-2'>
                <Button onClick={isFullscreen ? exitFullscreen : enterFullscreen} text size='small' icon={isFullscreen ? 'pi pi-arrow-down-left-and-arrow-up-right-to-center' : 'pi pi-arrow-up-right-and-arrow-down-left-from-center'}></Button>

...
const QuestionExamSetting = () => {
    ...
    const sumScore = useMemo(() => {
        return Object.values(hashScore).reduce((pre: any, cur: any) => pre + +cur, 0) as number
    }, [hashScore])
    ...
    const disabled = useMemo(() => {
        return Object.values(hashScore).reduce((pre: any, cur: any) => pre + +cur, 0) !== 10 || Object.values(hashScore).some((item: any) => item < 0)
    }, [hashScore])
        ...
        <div>
            <div className='tw-flex'>
                <div className='tw-w-full md:tw-w-[70%] px-2 tw-space-y-2 tw-max-h-[500px] tw-overflow-y-auto'>
                    {content?.questions?.map((item: IQuestion, index: number) => <div
                        key={item.id}
                    ...
                    >
                        ...
                        <div className='tw-flex tw-items-center tw-gap-2'>
                            <div>
                                <div>Điểm</div>
                                <InputNumber value={hashScore[item.id]} onChange={(e) => setHashScore({ ...hashScore, [item.id]: e.value })} className='tw-w-[80px]' inputClassName='tw-w-[80px]' minFractionDigits={2} />
                            </div>
                            <div>
                                ...
                                <Button
                                    ...
                                    onClick={() => {
                                        const newScore = 10 - sumScore + (hashScore[item.id] || 0);
                ...
                <div className="tw-w-full md:tw-w-[30%] md:tw-border-l-2 md:tw-pl-4">
                    ...
                    <p>
                        <strong>Còn lại:</strong>{((10 - (sumScore as number))).toFixed(2)}%
                    </p>
                </div>

            </div>
            <div className="tw-mt-8">
                Bạn đang thiết lập điểm, ấn <strong>Lưu thay đổi</strong>{" "}
                để cập nhật
            </div>
            <div className="tw-flex tw-justify-end tw-mt-8">
                <Button onClick={handleApply} disabled={disabled} icon={"pi pi-save"} label="Lưu thay đổi"></Button>
            </div>
        </div>
    )
}

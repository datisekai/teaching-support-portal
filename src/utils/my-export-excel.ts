...
export function exportExcel(
  ...
  cell3.alignment = { horizontal: "right", vertical: "middle", wrapText: true };

  worksheet.mergeCells("F2:G2");
  const cell4 = worksheet.getCell("F2");
  cell4.value = "Độc lập - Tự do - Hạnh phúc";
  cell4.font = { bold: true, size: 12 };
  cell4.alignment = { horizontal: "right", vertical: "middle", wrapText: true };
  ...
  // Add schema headers starting from row 8
  schemas.forEach((schema: any, index: number) => {
    const cell = worksheet.getCell(8, index + 1); // Row 8, column based on index
  ...
  data.forEach((item: any, rowIndex: number) => {
    const row = worksheet.getRow(9 + rowIndex); // Start from row 9
    schemas.forEach((schema: any, colIndex: number) => {
      row.getCell(colIndex + 1).value = item[schema.prop];
    });
  });

  // Adjust column widths
  schemas.forEach((schema: any, index: number) => {
    worksheet.getColumn(index + 1).width = schema.type === "number" ? 10 : 30;
  });

...
const ResultMultipleChoice = (props: any) => {
  ...
  useEffect(() => {
    if (data && data.questionTemp) {
      const score = updatedExam?.[data.id]
        ? +updatedExam?.[data.id]
        : data?.grade || 0;
  ...
  return (
    <div>
      <div className="tw-text-sm tw-mb-2">
        (Thay đổi kết quả sẽ không làm ảnh hưởng đến đáp án mà sinh viên chọn.)
      </div>
      <SelectButton
        value={choice}
        onChange={(e) => handleChangeChoice(e.value)}
        optionLabel="label"
        optionValue="value"
...
const ResultCode = (props: any) => {
  ...
  return (
    <div className="tw-mt-2 tw-flex tw-items-center tw-gap-2">
      <InputNumber
        onChange={(e) => setGrade(e.value as number)}
        disabled={!editable}
        className="tw-w-[80px]"
...
const PreviewResult: React.FC<Props> = ({ data }) => {
  return (
    <div className="tw-mb-4">
      {data?.questionTemp.type == QuestionType.MULTIPLE_CHOICE && (
        <ResultMultipleChoice key={data.id} data={data} />
      )}
      {(data?.questionTemp.type == QuestionType.CODE ||
        data?.questionTemp.type == QuestionType.CODE_HTML) && (
        <ResultCode key={data.id} data={data} />
      )}
    </div>
  );
};
